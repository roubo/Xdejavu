...
class Dejavu(object):
    ...
    def getchannelamount(self, filename):
        """
           we need know how many items to show
        """
        songname, extension = os.path.splitext(os.path.basename(filename))
        channels, Fs = decoder.read(filename, self.limit)
        channel_amount = len(channels)
        return channel_amount

    def get2doriginaldata(self, filename, channelnum, song_name=None):
        songname = decoder.path_to_songname(filename)
        song_name = song_name or songname
        channels, Fs = decoder.read(filename, self.limit)
        for channeln, channel in enumerate(channels):
            if channelnum == channeln + 1 :
                arry, local_maxima = fingerprint.fingerprintwithouthash(channel, Fs=Fs)
                return arry, local_maxima;
        return None, None
